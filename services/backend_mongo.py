...
class BackendMongoImpl:
    def __init__(self):
        try:
            ...
            logging.error("Can't connect to MongoDB server (", settings.MONGODB_SERVER['HOST'], ' ',
                          settings.MONGODB_SERVER['PORT'], " )")
    ...
    def getUserUIDForObject(self, lbeObjectTemplate, UID):
        searchResult = self.handler.searchDocuments(lbeObjectTemplate.name,
                                                    {'attributes.uid': UID, 'status': {'$nin': [OBJECT_STATE_DELETED]}})
    ...
    def updateObject(self, lbeObjectTemplate, lbeObjectInstance, changes):
        ...
        return self.handler.updateDocument(lbeObjectTemplate.name, lbeObjectInstance,
                                           {'_id': lbeObjectInstance.name.__str__()}, {'$set': changes})
    ...
    def searchObjects(self, lbeObjectTemplate, index=0, size=0):
        ...
        'status': {'$gt': OBJECT_STATE_INVALID}}, index, size))
